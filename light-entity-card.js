...
`;class it extends et{static get properties(){return{hass:Object,config:Object}}static get featureNames(){return{brightness:1,colorTemp:2,effectList:4,color:16,whiteValue:128}}static get cmdToggle(){return{on:"turn_on",off:"turn_off"}}static get entityLength(){return{light:10,switch:1}}getCardSize(){if(!this.config||!this.__hass||!this.__hass.states[this.config.entity])return 1;let t=0;const e=this.__hass.states[this.config.entity];return Array.isArray(e.attributes.entity_id)?e.attributes.entity_id.forEach(e=>t+=this.getEntityLength(e)):t+=this.getEntityLength(e.attributes.entity_id),this.config.group&&(entityLength*=.8),parseInt(entityLength)}getEntityLength(t){return/^light\./.test(t)?it.entityLength.light:/^switch\./.test(t)?it.entityLength.switch:0}get styles(){return st}constructor(){super()}get language(){return this.__hass.resources[this.__hass.language]}isEntityOn(t){return"on"===t.state}updated(){this._isUpdating=!1,this._shownStateObjects.forEach(t=>{const e=this.generateColorPickerId(t),s=this.shadowRoot.querySelectorAll(`#${e}`);if(s.length){const e=t.attributes.hs_color&&t.attributes.hs_color[0]||0,i=t.attributes.hs_color&&t.attributes.hs_color[1]/100||0;s[0].desiredHsColor={h:e,s:i}}})}setConfig(t){if(!t.entity)throw Error("entity required.");this.config={group:!1,persist_features:!1,...t}}render(){const t=this.__hass.states[this.config.entity];if(!t)throw Error(`Invalid entity: ${this.config.entity}`);return this._isUpdating=!0,this._shownStateObjects=this.getEntitiesToShow(t),this._shownStateObjects.reduce((t,e)=>t=V`
        ${t}
        ${this.createCard(e)}
      `,V`<style>${this.styles}</style>`)}getEntitiesToShow(t){return t.attributes.entity_id&&Array.isArray(t.attributes.entity_id)?t.attributes.entity_id.map(t=>this.__hass.states[t]):[t]}createCard(t){return V`
    ...
    `}createHeader(t){const e=this.config.header||t.attributes.friendly_name||t.entity_id;return V`
    ...
    `}createBrightnessSlider(t){return this.dontShowFeature("brightness",t)?V``:V`
    ...
    `}createColorTemperature(t){return this.dontShowFeature("colorTemp",t)?V``:V`
    ...
    `}createWhiteValue(t){return this.dontShowFeature("whiteValue",t)?V``:V`
    ...
    `}createEffectList(t){if(!this.config.persist_features&&!this.isEntityOn(t))return V``;let e=t.attributes.effect_list||[];if(this.config.effects_list&&Array.isArray(this.config.effects_list))e=this.config.effects_list;else if(this.config.effects_list&&this.hass.states[this.config.effects_list]){const t=this.hass.states[this.config.effects_list];e=t.attributes&&t.attributes.options||[]}else if(this.dontShowFeature("effectList",t))return V``;const s=e.map(t=>V`<paper-item>${t}</paper-item>`),i=e.indexOf(t.attributes.effect),r=this.language["ui.card.light.effect"];return V`
    ...
    `}createColorPicker(t){return this.dontShowFeature("color",t)?V``:V`
    ...
    `}dontShowFeature(t,e){return!(it.featureNames[t]&e.attributes.supported_features)||(!this.config.persist_features&&!this.isEntityOn(e)||void 0)}generateColorPickerId(t){return`light-entity-card-${t.entity_id.replace(".","-")}`}setColorPicker(t,e){this.callEntityService({hs_color:[t.detail.hs.h,100*t.detail.hs.s]},e)}setBrightness(t,e){const s=parseInt(t.target.value);isNaN(s)||parseInt(e.attributes.brightness)===s||this.callEntityService({brightness:s},e)}setColorTemp(t,e){let s=parseInt(t.target.value);isNaN(s)||parseInt(e.attributes.color_temp)===s||this.callEntityService({color_temp:s},e)}setWhiteValue(t,e){const s=parseInt(t.target.value);isNaN(s)||parseInt(e.attributes.white_value)===s||this.callEntityService({white_value:s},e)}setToggle(t,e){const s=this.isEntityOn(e)?it.cmdToggle.off:it.cmdToggle.on;this.callEntityService({},e,s)}setEffect(t,e){this.callEntityService({effect:t.detail.value},e)}callEntityService(t,e,s){if(this._isUpdating)return;const i=e.entity_id.split(".")[0];this.hass.callService(i,s||it.cmdToggle.on,{entity_id:e.entity_id,...t})}}customElements.define("light-entity-card",it)}();
